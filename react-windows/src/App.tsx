...
function App() {
    ...
    // Open folder (opens in explorer for Electron, copies to clipboard for web)
    const handleOpenFolder = async () => {
        if (exportedPosterPath) {
            if (window.electronAPI?.isElectron) {
                // Electron: open folder in file explorer
                await window.electronAPI.openFolder(exportedPosterPath)
                showToast('Opened in File Explorer')
            } else {
                // Web: copy path to clipboard
    ...
    // Download poster to user's PC (with native save dialog in Electron)
        ...
        try {
            ...
            if (window.electronAPI?.isElectron) {
                ...
                if (savePath) {
                    ...
                    const result = await window.electronAPI.saveFile(posterImageUrl, savePath)
                    if (result.success) {
                        showToast(`Saved to ${result.path}`)
                        setExportedPosterPath(result.path || savePath)
                    } else {
